...
import com.port.logistics.holdings.data.chart.ScatteredGroceriesAmountData;
 ...
 * @since 1.0
 */
public class ScatteredGroceriesAmountFragment extends BaseChartFragment {
    ...
    protected void onBuildChart(final BaseSupportUtils chartUtil) {
        ...
        // 新建集装箱数据类型
        ScatteredGroceriesAmountData scatteredGroceriesAmountData = new ScatteredGroceriesAmountData();
        ...
        pullChartData.setWorkBackListener(new WorkBack<IChartData>() {
            ...
            public void doEndWork(boolean state, IChartData data) {
                if (state) {
                    fillChartDataSet(chartUtil, (ScatteredGroceriesAmountData) data);
                    drawChartView();
                }
            }
        });

        pullChartData.beginExecute(scatteredGroceriesAmountData);
    }
     ...
     * @param scatteredGroceriesAmountData 图表数据集合
     */
    private void fillChartDataSet(BaseSupportUtils chartUtil, ScatteredGroceriesAmountData scatteredGroceriesAmountData) {

        // X坐标值
        double[] XValues = new double[scatteredGroceriesAmountData.getDates().length];
        ...
        // X轴显示的文本
        String[] XLabel = scatteredGroceriesAmountData.getDates();
        // Y轴显示的文本
        //String[] YLabel = new String[]{"0" , "50" , "100" , "150"};

        // 数据的纵坐标实际值
        double[] amount = scatteredGroceriesAmountData.getAmount();
        ...
        chartUtil.setChartTitle("本年散杂货代理量趋势分析");
        chartUtil.setXTitle("月度");
        chartUtil.setYTitle("万吨");
        ...
        clickPointTexts = scatteredGroceriesAmountData.getAmountPercent();
